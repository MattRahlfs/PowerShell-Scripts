...
Write-Output "Checking if the spooler is running"
    try{
        If(Get-Service -ComputerName $ComputerName -Name Spooler | Where Status -eq "Running"){
           Write-Output "Spooler service is running, stopping service"
           Invoke-Command -ComputerName $ComputerName -scriptblock{Stop-Service -Force -Name Spooler}
           If(Get-Service -ComputerName $ComputerName -Name Spooler | Where Status -eq "Stopped"){
              
              Write-Output "Service is stopped, deleting files from C:\Windows\System32\spool\PRINTERS\"
              Invoke-Command -ComputerName $ComputerName -scriptblock{$jobLoc = "C:\Windows\System32\spool\PRINTERS\"
                                                               Remove-Item -Path $jobLoc -recurse}
            }
        }
        Else{Write-Output "Spooler service is not running"}
        Write-Output "Starting the spooler"
        Invoke-Command -computername $ComputerName -scriptblock{Start-Service -Name Spooler}}
