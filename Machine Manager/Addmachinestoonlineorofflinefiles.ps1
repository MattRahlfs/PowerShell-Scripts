...
If(Test-Connection -ComputerName $machine -Count 1 -TimeToLive 10 -ErrorAction SilentlyContinue){


$machine | Out-File -FilePath "\\mercyhome\hm\mrahlfs1\Connected computers.txt" -Append

}
 Else{$machine | Out-File -FilePath "\\mercyhome\hm\mrahlfs1\failed to connect to machines.txt" -Append} 
}
}
